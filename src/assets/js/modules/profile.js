...
const serverDomain = 'http://u.7enov.com.ua:4000';
...
$(document).ready( () => {
    ...
    $(loadMoreBtn).click( () => {
        ...
        page++;
        if ( offset * page > total ) $('#loadMore').fadeOut();
    });

    // On change sorting
    $('#profileSort').change( () => {
        const sortingName = $('#profileSort').val();
    ...
    // Toggle favorite
    $('.listing-item__favorite').click( e => {
        const clickedStar = $(e.target).closest('.listing-item__favorite');
        // const adId = clickedStar.dataset.adid;

        console.log(clickedStar);

        // toggleFavorite(adId, userId, token, deviceId)
    });
});
